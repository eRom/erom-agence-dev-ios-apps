...
| `ios-simulator-browser` | portée vers le Browser pane de Desktop (`.claude/launch.json`, `bunx serve-sim@0.1.46`, `autoPort: false`). Sources : doc Desktop, README serve-sim (section « Claude Code Desktop »), `dist/serve-sim.js` 0.1.46 qui ne lit que `--port`. Voie CLI testée le 2026-09-12 sur iPhone 17 Pro / iOS 26.5 : preview sur 3200, image en direct dans Chrome, `tap` et `button home` reçus (`event-log`). Voie Desktop (Browser pane) validée par Romain le 2026-09-12, session `2a6c5a28-adad-4887-b456-c908a232f588` |
